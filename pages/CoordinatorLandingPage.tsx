...
export const CoordinatorLandingPage = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await fetch('http://192.168.43.78:3002/coordinatorlandingpage', {
          method: 'GET',
  ...
  const handleAddRep = async () => {
    try {
      const response = await fetch('http://192.168.43.78:3002/studentrepsignup', {
        method: 'POST',
        headers: {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.43.78:3002/changepasswordcoordinator', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
