...
export const TrialsConfirmation = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const response = await fetch('http://192.168.43.78:3002/getMyTrialEvents', {
          method: 'GET',
  ...
  const handleCreateEvent = async () => {
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await fetch('http://192.168.43.78:3002/createTrialEvent', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteEvent = async (id) => {
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await fetch(`http://192.168.43.78:3002/deleteTrialEvent/${id}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
