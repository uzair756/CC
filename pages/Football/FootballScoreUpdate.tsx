...
export const FootballScoreUpdatePage = ({ route, navigation }) => {
  ...
  const swapPlayers = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.139.169:3002/swapPlayers', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          matchId: match._id,
          reservedPlayerId: selectedReservedPlayer._id,
          playingPlayerId: selectedPlayingPlayer._id,
        }),
      });
  ...
  const handleStart = async (matchId) => {
    try {
      ...
      const response = await fetch('http://192.168.139.169:3002/startmatchfootball', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const handleStop = async (matchId) => {
    try {
      ...
      const response = await fetch('http://192.168.139.169:3002/stopmatchfootball', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ matchId }),
      });
  ...
  const handleGoalIncrement = async (playerId, team) => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.139.169:3002/updateGoalFootball', {
        method: 'POST',
        headers: {
  ...
  const handleEndHalf1 = async () => {
    if (matchDetails.half === 1) {
      try {
        ...
        const response = await fetch('http://192.168.139.169:3002/updateHalffootball', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
