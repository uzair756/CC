...
import { View, Text, StyleSheet, ScrollView, Image, TouchableOpacity, TouchableHighlight, ActivityIndicator , ImageBackground} from 'react-native';
...
export const LiveMatchesScreen = () => {
  ...
  useEffect(() => {
    const fetchLiveMatches = async () => {
      setLoading(true);
      try {
        const response = await fetch(`http://192.168.1.21:3002/livematches?sportCategory=${selectedSport}`);
              ...
              >
                <View style={styles.matchCard}>
                  ...
                  {selectedSport === "Cricket" && (
                      <Text style={styles.inningText}>{match.inning === 1 ? "1st Inning" : match.inning === 2 ? "2nd Inning" : match.inning}</Text>
                    )}
                    ...
                    {selectedSport === "Cricket" ? (
                      <>
                        <Text style={styles.score}>
                          {match.scoreT1}/{match.T1wickets} - {match.scoreT2}/{match.T2wickets}
                        </Text>
                      </>
  ...
  <View>
    <Text style={styles.oversText}>
      Overs: {match.inning === 1 ? match.oversInning1 : match.oversInning2}
    </Text>
    
    <View style={styles.runsContainer}>
      {(match.inning === 1 ? match.runsInning1 : match.runsInning2)
...
{selectedSport === "Cricket" && match.nominationsT1 && match.nominationsT2 && (
  <>
    {/* Active Batsmen */}
    {/* Active Batsmen */}
 {/* Active Bowler */}
 <Text style={styles.playerText}>
  Active Batsman of {match?.inning === 1 ? match.FirstInningBattingTeam : match.SecondInningBattingTeam}
</Text>
{[...match.nominationsT1, ...match.nominationsT2]
  ...
  <View style={styles.rightContainer}>
    ...
    <Text style={styles.goalsText}>Runs Scored: {player.runsScored}</Text>
           <Text style={styles.goalsText}>Balls Faced: {player.ballsFaced.length}</Text>
           <Text style={styles.goalsText}>
       Strike Rate: {((player.runsScored / player.ballsFaced.length) * 100 || 0).toFixed(2)}
     </Text>

    <View style={styles.ballsContainer}>
      {player.ballsFaced.slice(-6).map((ball, idx) => (
    ...
    {/* Active Bowler */}
    <Text style={styles.playerText}>
  Active Bowler of {match?.inning === 1 ? match.FirstInningBowlingTeam : match.SecondInningBowlingTeam}
</Text>
  ...
  <View style={styles.rightContainer}>
     ...
     <Text style={styles.goalsText}>Wickets Taken: {player.wicketsTaken}</Text>
           <Text style={styles.goalsText}>Overs: {formatBowlerOvers(player.ballsBowled)}</Text>
           <Text style={styles.goalsText}>
  Runs Conceded: {player.ballsBowled.reduce((acc, val) => 
  ...
  , 0)}
            ...
            <Text style={styles.goalsText}>
             Economy Rate: {(
               (player.ballsBowled.reduce((acc, val) => acc + (["W", "NB", "WD"].includes(val) || val.endsWith("B") ? 0 : Number(val)), 0) /
               (player.ballsBowled.filter(ball => !["NB", "WD"].includes(ball) && !ball.endsWith("B")).length / 6)
               ) || 0
             ).toFixed(2)}
           </Text>

    <View style={styles.ballsContainer}>
      {player.ballsBowled.slice(-6).map((ball, idx) => (
        <View key={idx} style={styles.ballBox}>
          <Text style={styles.ballText}>{ball}</Text>
        </View>
      ))}
                  ...
                  <Text style={styles.winnerText}>{getWinner(match.result)}</Text>
                </View>
              </TouchableHighlight>
            ))
          )}
        ...
        {loading && matches.length > 0 && <ActivityIndicator size="small" color="#007BFF" />}
      </View>
    </View>
  );
};
