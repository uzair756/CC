...
import { View, Text, Alert, StyleSheet, ScrollView, TouchableOpacity, BackHandler,Modal, Image ,StatusBar} from 'react-native';
...
export const BasketballScoreUpdatePage = ({ route,navigation }) => {
  ...
  const swapPlayers = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.139.169:3002/swapPlayersbasketball', {
        method: 'POST',
  ...
  const handleStart = async (matchId) => {
    try {
      ...
      const response = await fetch('http://192.168.139.169:3002/startmatchbasketball', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const handleStop = async (matchId) => {
    try {
      ...
      const response = await fetch('http://192.168.139.169:3002/stopmatchbasketball', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ matchId }),
      });
          ...
          Alert.alert('Success', 'Match stopped successfully', [
            {
              ...
              onPress: () => {
                ...
                if (matchDetails.pool === 'final') {
                  navigation.navigate('BestBasketballPlayerPage');
                } else {
  ...
  const handlePointIncrement = async (playerId, team, value) => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.139.169:3002/updateGoalbasketball', {
        method: 'POST',
        headers: {
  ...
  const handleEndQuarter = async (currentQuarter) => {
    try {
        ...
        const response = await fetch('http://192.168.139.169:3002/updateHalfbasketball', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
...
const handleEnd4thQuarter = async () => {
  try {
      ...
      const response = await fetch('http://192.168.139.169:3002/updateHalf4thbasketball', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    ...
    <LinearGradient colors={['#f5f7fa', '#e4e8f0']} style={styles.container}>
      ...
      <View style={styles.actionButtonsContainer}>
        ...
        {matchDetails?.quarter === 4 && (
          ...
          >
            <Icon name="pause" size={16} color="#fff" />
            <Text style={styles.actionButtonText}>End Q4</Text>
          </TouchableOpacity>
        )}
              ...
              <View key={index} style={styles.playerCard}>
                ...
                <View style={styles.pointButtonsContainer}>
                  {[1, 2, 3].map((value) => (
                    ...
                    >
                      <Text style={styles.pointButtonText}>+{value}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
