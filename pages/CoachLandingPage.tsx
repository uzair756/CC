...
export const CoachLandingPage = ({ navigation }) => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        const response = await fetch('http://192.168.43.78:3002/coachlandingpage', {
          method: 'GET',
  ...
  const handleAddCoordinator = async () => {
    ...
    try {
      const response = await fetch('http://192.168.43.78:3002/addcoordinator', {
        method: 'POST',
        headers: {
  ...
  const handleAddRef = async () => {
    ...
    try {
      ...
      const token = await AsyncStorage.getItem('token');
      const response = await fetch('http://192.168.43.78:3002/addref', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  const fetchSportRules = async (sport) => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await fetch(`http://192.168.43.78:3002/getrules/${sport.toLowerCase()}`, {
        method: 'GET',
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const updateSportRules = async () => {
    try {
      ...
      const response = await fetch(`http://192.168.43.78:3002/updaterules/${selectedSport.toLowerCase()}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
  ...
  const handleChangePassword = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await fetch('http://192.168.43.78:3002/changepasswordcoach', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
