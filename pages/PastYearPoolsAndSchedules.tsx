...
export const PastYearPoolsAndSchedules = () => {
  ...
  //       `http://192.168.43.78:3002/get-schedules?sport=${selectedSport}&year=${selectedYear}`,
  //       {
  //         headers: {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `http://192.168.43.78:3002/get-schedules?sport=${encodeURIComponent(selectedSport)}&year=${encodeURIComponent(selectedYear)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
