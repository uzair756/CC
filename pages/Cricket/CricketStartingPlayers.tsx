...
export const CricketStartingPlayers = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchFirstInningTeams = async () => {
            try {
                ...
                const response = await fetch(`http://192.168.1.24:3002/getFirstInningTeams?matchId=${match._id}`, {
        ...
        const fetchPlayers = async (battingTeam, bowlingTeam) => {
            try {
                const token = await AsyncStorage.getItem("token");
                const response = await fetch(`http://192.168.1.24:3002/getPlayers?matchId=${match._id}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                const data = await response.json();
    ...
    const handleSubmit = async () => {
        ...
        try {
            const token = await AsyncStorage.getItem("token");
            const response = await fetch("http://192.168.1.24:3002/updatePlayingStatus", {
                method: "POST",
