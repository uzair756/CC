...
export const CricketScoreUpdateSecondInning = ({ route,navigation }) => {
  ...
  useEffect(() => {
      if (matchDetails?.oversInning2 % 1 === 0 && matchDetails?.oversInning2 !== 0) {
    ...
    const fetchMatchDetails = async () => {
        try {
            ...
            if (data.success) {
                ...
                const playingT1 = [], reservedT1 = [], activeBatsmenT1 = [], activeBowlerT1 = [];
                const playingT2 = [], reservedT2 = [], activeBatsmenT2 = [], activeBowlerT2 = [];
                ...
                } else if (activeBatsmenT2.length === 2 && activeBowlerT1.length === 1) {
...
const handleWicketSelection = async (newBatsman) => {
  ...
  try {
    const token = await AsyncStorage.getItem("token");
    const response = await fetch("http://192.168.1.21:3002/swapPlayerscricket2ndInning", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
...
const handleOverSelection = async (newBowler) => {
  ...
  try {
    const token = await AsyncStorage.getItem("token");
    const response = await fetch("http://192.168.1.21:3002/swapbowlercricket2ndInning", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
...
const handleByesIncrement = async (team, byes) => {
  ...
  try {
      ...
      const response = await fetch('http://192.168.1.21:3002/updateByesCricket2ndInning', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`,
...
try {
    ...
    const response = await fetch('http://192.168.1.21:3002/updateExtrasCricket2ndInning', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
