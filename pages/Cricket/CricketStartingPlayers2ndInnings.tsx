...
export const CricketStartingPlayers2ndInnings = ({route, navigation}) => {
  ...
  useEffect(() => {
    const fetchSecondInningTeams = async () => {
      try {
        ...
        const response = await fetch(
          `http://192.168.139.169:3002/getSecondInningTeams?matchId=${match._id}`,
          {
    ...
    const fetchPlayers = async (battingTeam, bowlingTeam) => {
      try {
        const token = await AsyncStorage.getItem('token');
        const response = await fetch(
          `http://192.168.139.169:3002/getPlayers2ndinning?matchId=${match._id}`,
          {
            headers: {Authorization: `Bearer ${token}`},
          },
        );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const token = await AsyncStorage.getItem('token');
      const response = await fetch(
        'http://192.168.139.169:3002/updatePlayingStatus2ndInning',
        {
          method: 'POST',
          headers: {
