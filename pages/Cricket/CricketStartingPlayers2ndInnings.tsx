...
export const CricketStartingPlayers2ndInnings = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const fetchSecondInningTeams = async () => {
            try {
                ...
                const response = await fetch(`http://192.168.1.21:3002/getSecondInningTeams?matchId=${match._id}`, {
                ...
                if (data.success) {
                    ...
                    fetchPlayers(data.SecondInningBattingTeam, data.SecondInningBowlingTeam);
                } else {
        ...
        const fetchPlayers = async (battingTeam, bowlingTeam) => {
            try {
                const token = await AsyncStorage.getItem("token");
                const response = await fetch(`http://192.168.1.21:3002/getPlayers2ndinning?matchId=${match._id}`, {
                    headers: { Authorization: `Bearer ${token}` },
                });
                const data = await response.json();
                ...
                if (data.success) {
                    ...
                    if (hasActivePlayer) {
                        navigation.replace("CricketScoreUpdateSecondInning", { match: { ...match, ...data } });
                        return;
                    }
    ...
    const handleSubmit = async () => {
        ...
        try {
            const token = await AsyncStorage.getItem("token");
            const response = await fetch("http://192.168.1.21:3002/updatePlayingStatus2ndInning", {
                method: "POST",
