...
export const RecentMatchesScreen = () => {
  ...
  //     try {
  //       const response = await fetch('http://192.168.100.4:3002/recentmatches'); // Adjust URL accordingly
  ...
  useEffect(() => {
      const fetchUpcomingMatches = async () => {
        setLoading(true);
        try {
          const response = await fetch(`http://192.168.100.4:3002/recentmatches?sportCategory=${selectedSport}`);
          const data = await response.json();
