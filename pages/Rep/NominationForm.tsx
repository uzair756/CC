...
export const NominationForm = ({ route }) => {
  ...
  useEffect(() => {
    const fetchNominationData = async () => {
      ...
      try {
        const response = await fetch(`http://10.4.36.23:3002/getNominationForm/${sport}`, {
          method: 'GET',
          headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (nominationData && nominationData._id) {
        response = await fetch(`http://10.4.36.23:3002/updateNominationForm/${sport}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
        ...
        response = await fetch(`http://10.4.36.23:3002/submitNominationForm/${sport}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
